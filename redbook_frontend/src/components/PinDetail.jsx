...
const PinDetail = ({ user }) => {
    ...
    const fetchPinDetails = () => {
        ...
        if (query) {
            client.fetch(query)
                .then((data) => {
                    setPinDetail(data[0])
                    // codes below is for recommandations ralated to other similar pins
                    if (data[0]) {
    ...
    // only when {pinId} CHANGES, the useEffect triggers func fetchPinDetails()

    if (!pinDetail) return <Spin message='Loading pins...' />

    return (
        <>
            <div className='flex xl-flex-row flex-col m-auto bg-white '
                style={{
                    maxWidth: '1500px', borderRadius: '32px'
                }}>
                ...
                <div className='w-full p-5 flex-1 xl:min-w-620'>
                    <div className='flex items-center flex-wrap justify-between'>
                        ...
                        <Link to={`user-profile/${pinDetail?.postedBy?._id}`} className='flex flex-shrink-0 w-full gap-2 mt-5 items-center bg-white rounded-lg'>
                            <img
                        ...
                        <div className='flex-shrink-0 w-full mt-4 border-t'>
            ...
            ) : (<Spin message='Loading more pins...' />)}
        </>
    )
}
